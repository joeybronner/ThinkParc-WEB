...
if(isset($_POST["submit"])) {
    ...
    if($check !== false) {
			...
			if ($_FILES["fileToUpload"]["size"] > 500000) {
				...
				header('Location: docstechniquesvehicules.php?add=failed'); 
			} else {
				...
				if($imageFileType != "jpg" && $imageFileType != "JPG" && $imageFileType != "png" && $imageFileType != "PNG" && $imageFileType != "JPEG" && $imageFileType != "jpeg" && $imageFileType != "gif" ) {
					...
					if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
						...
						header('Location: docstechniquesvehicules.php?add=success'); 
					} else {
        ...
        $_SESSION['fct_message'] = "Veuillez sélectionner une fichier.";
		header('Location: docstechniquesvehicules.php?add=failed');		
    }
}
