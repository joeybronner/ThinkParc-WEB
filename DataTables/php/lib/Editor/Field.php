...
class Field extends DataTables\Ext {
	...
	/**
	 * Get an aliased name for the database field. This effectively escapes the
	 * period that can be used in the field, to allow an alias to be valid.
	 *  @return string The name of the aliased db field
	 *  @internal
	 */
	public function dbAlias ()
	{
		return str_replace( '.', '___', $this->_dbField );
	}
			...
			if ( $this->_getFormatter ) {
				if ( is_string( $this->_getFormatter ) ) {
					...
					if ( strpos($this->_getFormatter, "Format::") === 0 ) {
						return call_user_func( "\\DataTables\\Editor\\".$this->_getFormatter, $data[ $this->dbAlias() ], $data, $this->_getFormatterOpts );
					}
					return call_user_func( $this->_getFormatter, $data[ $this->dbAlias() ], $data, $this->_getFormatterOpts );
				}
				$getFormatter = $this->_getFormatter;
				return $getFormatter( $data[ $this->dbAlias() ], $data, $this->_getFormatterOpts );
			}
			return $data[ $this->dbAlias() ];
		}
		else {
			...
			if ( $this->_setFormatter ) {
				if ( is_string( $this->_setFormatter ) ) {
					...
					if ( strpos($this->_setFormatter, "Format::") === 0 ) {
						return call_user_func( "\\DataTables\\Editor\\".$this->_setFormatter, $data[ $this->dbAlias() ], $data, $this->_setFormatterOpts );
					}
					return call_user_func( $this->_setFormatter, $data[ $this->dbAlias() ], $data, $this->_setFormatterOpts );
				}
				$setFormatter = $this->_setFormatter;
				return $setFormatter( $data[ $this->dbAlias() ], $data, $this->_setFormatterOpts );
			}
